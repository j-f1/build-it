...
export default st.handleHover(st.handleActive(function Copy (props) {
  const { text } = props
  const styles = st({
    default: {
      container: {
  ...
  return <span style={styles.container} title='Click to copy' onClick={copy(text)}>
    {props.after || <code>{text}</code>}{' '}
    <svg style={styles.svg} width='14' height='16' viewBox='0 0 14 16'>
      {/* octicons/clippy */}
